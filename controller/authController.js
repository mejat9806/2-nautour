...
function createSendToken(user, statusCode, res) {
  ...
  const RefreshTokenOptions = {
    expires: new Date(
      Date.now() +
        process.env.REFRESH_JWT_COOKIES_EXPIRES_IN * 24 * 60 * 60 * 1000,
    ),
    secure: true,
    httpOnly: true,
    sameSite: 'none',
...
export const protect = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    //? this header only for API the page use cookies // for first time login
...
export const forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    //   message,
    // });
    const resetURL = `${req.protocol}://${req.get('host')}/api/v1/users/resetPassword/${resetToken}`; //this way to get the url from the request
